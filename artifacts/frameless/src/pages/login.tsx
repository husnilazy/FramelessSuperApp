// artifacts/frameless/src/pages/login.tsx
// Fully inline-styled — works regardless of Tailwind compile status
import { useState, useRef, useEffect } from "react";
import { useLocation } from "wouter";
import { useToast } from "@/hooks/use-toast";
import { useAuth } from "@/lib/auth";

// ── Mesh background (reused pattern from landing) ─────────────────────────────
const CSS = `
...
function Mesh() {
  return (
    <div style={{ position:"absolute", inset:0, overflow:"hidden", pointerEvents:"none" }}>
      <div style={{ position:"absolute", width:"70%", height:"70%", top:"-20%", left:"-15%",
        background:`radial-gradient(ellipse at center, ${OR}50 0%, ${OR}22 45%, transparent 68%)`,
        filter:"blur(60px)", animation:"b1 18s ease-in-out infinite" }} />
      <div style={{ position:"absolute", width:"65%", height:"65%", top:"5%", right:"-20%",
        background:"radial-gradient(ellipse at center, #7c3aed44 0%, #7c3aed18 50%, transparent 72%)",
        filter:"blur(70px)", animation:"b2 22s ease-in-out infinite" }} />
      <div style={{ position:"absolute", width:"55%", height:"55%", bottom:"-20%", left:"22%",
        background:"radial-gradient(ellipse at center, #2563eb35 0%, transparent 70%)",
        filter:"blur(80px)", animation:"b3 26s ease-in-out infinite" }} />
      {/* grid */}
      ...
      <div style={{ position:"absolute", left:0, right:0, height:2,
        background:"linear-gradient(to right,transparent,rgba(255,106,32,.15),transparent)",
        animation:"scanline 8s linear infinite", pointerEvents:"none" }} />
      {/* vignette */}
      <div style={{ position:"absolute", inset:0,
        background:"radial-gradient(ellipse 100% 100% at 50% 50%,transparent 40%,rgba(10,10,12,.85) 100%)" }} />
    </div>
  );
}
      ...
      <div className="login-card" style={{
        ...
        <div style={{ textAlign:"center", marginBottom:36 }}>
          ...
          <div style={{ display:"inline-flex", alignItems:"center", justifyContent:"center",
            ...
            background:`linear-gradient(135deg, ${OR}, #e84d00)`,
            marginBottom:20, boxShadow:`0 0 40px ${OR}55` }}>
          ...
          <p style={{ fontSize:11, fontWeight:600, letterSpacing:".22em",
          ...
          <div style={{ display:"inline-flex", alignItems:"center", gap:6, marginTop:14,
            ...
            <span style={{ fontSize:9, fontWeight:700, letterSpacing:".18em",
              color:"rgba(74,222,128,.7)", textTransform:"uppercase" }}>System Online</span>
          </div>
        </div>
