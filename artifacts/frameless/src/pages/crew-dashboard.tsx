...
function CrewLogin({ onLogin }: { onLogin: (token: string, user: CrewUser) => void }) {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const data = await fetch("/api/crew/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, password }) }).then(r => r.json());
      if (!data.token) throw new Error(data.error || "Login gagal");
      localStorage.setItem("crew_token", data.token);
      localStorage.setItem("crew_user", JSON.stringify(data.crew || data.user));
      onLogin(data.token, data.crew || data.user);
...
function CrewDashboard({ user, onLogout }: { user: CrewUser; onLogout: () => void }) {
  const { toast } = useToast();
  const [tab, setTab] = useState<"overview" | "projects" | "tasks" | "chat" | "upload">("overview");
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const [proj, tsk, msgs] = await Promise.all([
        crewFetch("/api/projects").catch(() => []),
        crewFetch(`/api/crew/tasks?crewId=${user.id}`).catch(() => []),
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (file) {
        ...
        const token = getCrewToken();
        const r = await fetch("/api/uploads", { method: "POST", body: fd, headers: token ? { Authorization: `Bearer ${token}` } : {} });
